...
class CEMatrixSystem[F[_]](inner: SystemAlg[F])(using
    F: Temporal[F],
    S: AtomicState[F, CEMatrixSystem.CEMatrixSystemState],
  ...
  def newPassenger(passenger: Passenger): F[Unit] =
    for
      before <- F.monotonic
      _ <- S.update(CEMatrixSystem.enter(passenger, before))
      _ <- inner.newPassenger(passenger)
      after <- F.monotonic
      _ <- S.update(CEMatrixSystem.leave(passenger, after))
    yield ()
...
class CEFiberSystem[F[_]: Parallel](inner: SystemAlg[F])(using
    F: Spawn[F],
    S: AtomicState[F, CEFiberSystem.CEFiberSystemState[F]]
) extends SystemAlg[F]:
  def elevators: List[ElevatorAlg[F]] = inner.elevators
  ...
  def newPassenger(passenger: Passenger): F[Unit] =
    for
      ...
      _ <- S.update(x => x.copy(passengerFibers = fib :: x.passengerFibers))
    yield ()
  ...
  def start: F[Unit] =
    for
      ...
      _ <- S.update(x => x.copy(elevatorFibers = fibers))
    yield ()
